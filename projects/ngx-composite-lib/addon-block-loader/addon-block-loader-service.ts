...
export class AddonBlockLoaderService {
    ...
    async getPublicBaseURL(uuid: string){
        return {
            remoteEntry: 'https://cdn.pepperi.com/Addon/Public/ad909780-0c23-401e-8e8e-f514cc4f6aa2/0.0.8/addon.js',
            addonUUID: 'ad909780-0c23-401e-8e8e-f514cc4f6aa2',
            key: ''
        }
        //return  (await this.papiClient.addons.installedAddons.addonUUID(uuid).get()).PublicBaseURL;
    }

    async getAddonData(){
        const pagesAddon = await this.papiClient.addons.installedAddons.addonUUID(this.pagesUUID).get();
    }
